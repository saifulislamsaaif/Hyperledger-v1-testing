...
function getSubmitter(username, password, client, t, loadFromConfig, userOrg) {
	...
	console.log(password);
	console.log('@@@@@@@@@@@@end passwor@@@@@@@@@@@@@@@@@@@@@@@@')
	console.log('=====================Client info from util=======================');
	console.log(client);
...
module.exports.getCustomSubmitter = function(username, password, client, test, loadFromConfig, org) {
	if (arguments.length < 2) throw new Error('"client" and "test" are both required parameters');
	...
	return getSubmitter(username, password, client, test, fromConfig, userOrg);
};
